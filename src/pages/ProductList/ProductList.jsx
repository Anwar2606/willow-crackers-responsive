...
const ProductList = () => {
  ...
  return (
    <div className="product-list-container">
      ...
      <div className="button-row" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: '10px' }}>
  <button 
    className="select-all-button" 
    onClick={handleSelectAll} 
    style={{ flex: '1', padding: '10px 15px', fontSize: '16px', textAlign: 'center', minWidth: '120px' }}>
    {selectAll ? "Deselect All" : "Select All"}
  </button>
  <button 
  ...
  {filteredProducts.map((product) => (
    ...
    >
      <input
        ...
        checked={selectedProducts.includes(product.id)}
        onChange={(event) => handleSelectProduct(event, product.id)}
        style={{ marginRight: '15px' }} /* Adds space between checkbox and product details */
      />
      <div 
        className="product-info" 
        onClick={() => toggleDescription(product.id)} 
        style={{ flex: '1', cursor: 'pointer' }} /* Flexible width for details and clickable */
      >
        <div className="products-details">
          <div className="product-name" style={{ fontWeight: 'bold', marginBottom: '5px' }}>
            {product.name}
          </div>
          {product.expanded && (
            <div className="product-description" style={{ marginBottom: '5px' }}>
              {product.description}
            </div>
          )}
          ...
          <div className="product-price">
            Sales price: ₹{product.saleprice.toFixed(2)}
          </div>
        </div>
      </div>
      <div 
        className="product-actions" 
        style={{ display: 'flex', gap: '10px', flexShrink: '0' }} /* Ensures actions stay on one line */
      >
        <Link to={`/edit-product/${product.id}`}>
          <button 
